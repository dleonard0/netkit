...
write_netid_register(const char *transp)
{
	...
	snprintf(_errbuf, sizeof(_errbuf), "cannot find %s netid.", transp);
	...
	snprintf(_errbuf,sizeof(_errbuf), "cannot create %s service.", transp);
	print_err_message(tmpbuf);
	f_print(fout, "%s\t\texit(1);\n", sp);
	f_print(fout, "%s\t}\n", sp);
...
write_rest(void)
{
	...
	if (inetdflag) {
		...
		snprintf(_errbuf,sizeof(_errbuf), "could not create a handle");
		print_err_message("\t\t");
		f_print(fout, "\t\texit(1);\n");
		f_print(fout, "\t}\n");
		if (timerflag) {
	...
	snprintf(_errbuf, sizeof(_errbuf), "svc_run returned");
	print_err_message("\t");
	f_print(fout, "\texit(1);\n");
...
write_program(definition *def, const char *storage)
{
	...
	for (vp = def->def.pr.versions; vp != NULL; vp = vp->next) {
		...
		snprintf(_errbuf, sizeof(_errbuf), "unable to free arguments");
		print_err_message("\t\t");
		f_print(fout, "\t\texit(1);\n");
		f_print(fout, "\t}\n");
		print_return("\t");
