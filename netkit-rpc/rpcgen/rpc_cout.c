 ...
 */
char cout_rcsid[] = 
  "$Id: rpc_cout.c,v 1.8 1999/12/12 12:17:51 dholland Exp $";
...
emit_union(definition *def)
{
  ...
  for (cl = def->def.un.cases; cl != NULL; cl = cl->next) {
    ...
    if (!streq(cs->type, "void")) {
      objectlen = strlen(def->def_name) + strlen(format) +
	      strlen(cs->name) + 1;
      object = alloc(objectlen);
      if (isvectordef(cs->type, cs->rel)) {
	snprintf(object, objectlen, vecformat, def->def_name, 
		 cs->name);
      } else {
	snprintf(object, objectlen, format, def->def_name, 
		cs->name);
      }
				...
				if((dl->decl.prefix == NULL) && ((ptr=find_type(dl->decl.type))!= NULL) && 		 ((dl->decl.rel == REL_ALIAS)||(dl->decl.rel == REL_VECTOR))){
					...
					if(dl->decl.rel == REL_ALIAS) 
						...
						if(ptr->length != 1)
							snprintf(ptemp, 
								sizeof(ptemp),
								 " %s %s * %d",plus,dl->decl.array_max,ptr->length);
						else
							snprintf(ptemp, 
								 sizeof(ptemp),
								 " %s %s ",plus,dl->decl.array_max);
